...
import static com.zzy.android.custom.seekbar.SeekBar.INDICATOR_MODE_ALWAYS_SHOW;
...
//    private float maxPositiveValue, minPositiveValue;
    ...
    private void move(MotionEvent event) {
        ...
        if (currTouchSB == leftSB) {
            ...
            Log.d(TAGY, "currTouchSB == rightSB");
            if (tickMarkNumber > 1) {
                if (x > getLineRight()) {
                    percent = 1;
                } else {
    ...
    private static final String TAGY = "zzy";
    @Override
    public boolean onTouchEvent(MotionEvent event) {
        Log.d(TAGY, "onTouchEvent");
        if (!isEnable) return true;
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                Log.d(TAGY, "ACTION_DOWN");
            ...
            case MotionEvent.ACTION_MOVE:
                Log.d(TAGY, "ACTION_MOVE");
                move(event);
                break;
            case MotionEvent.ACTION_CANCEL:
                Log.d(TAGY, "ACTION_CANCEL");
                if (seekBarMode == SEEKBAR_MODE_RANGE) {
                    rightSB.setShowIndicatorEnable(false);
                }
                if (currTouchSB == leftSB){
