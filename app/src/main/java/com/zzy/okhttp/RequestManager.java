...
import static com.zzy.okhttp.constant.Constant.BASE_URL;

/**
 * Created by BCLZzy on 2018/6/14.
 */
...
public class RequestManager implements IRequestManager {
    ...
    public <T> Call requestGetByAsyn(String actionUrl, String path, final ReqCallBack<T> callBack) {
        try {
            String requestUrl = BASE_URL + String.format(actionUrl, path);
    ...
    public <T> Call requestGetByAsyn(String actionUrl, HashMap<String, String> paramsMap, final ReqCallBack<T> callBack) {
        ...
        try {
            ...
            String requestUrl = String.format("%s/%s?%s", BASE_URL, actionUrl, tempParams.toString());
            Log.d("okhttpactivity", requestUrl);
            final Request request = addHeaders().url(requestUrl).build();
            final Call call = mOkHttpClient.newCall(request);
            call.enqueue(new Callback() {
                @Override
